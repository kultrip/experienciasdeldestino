-- Experiencias del Destino - Database Schema
...
CREATE POLICY "Central can view all profiles"
  ...
  USING (
    EXISTS (
      ...
      WHERE user_id = auth.uid() AND role = 'central'
    )
  );
...
CREATE POLICY "Productores can create experiences"
  ...
  WITH CHECK (
    EXISTS (
      ...
      AND role IN ('productor', 'delegado', 'central')
    )
  );
...
CREATE POLICY "Users can view own bookings"
  ...
  USING (
    -- Experience owner can see bookings
    experience_id IN (
